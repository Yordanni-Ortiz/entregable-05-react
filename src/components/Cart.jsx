  ...
  useEffect(() => {
    ...
    setTotal(
      cartProducts.reduce(
        (accum, currentValue) =>
          accum +
          Number(currentValue.price) * currentValue.productsInCart.quantity,
        0
      )
    );
  }, [cartProducts]);
    ...
    if (getProducts) {
      setProducts(
        cartProducts.map((product) => {
          const productImgs = getProducts.find(
            (getProduct) => getProduct.id === product.id
          ).productImgs;
          return {
            ...product,
            productImgs,
      ...
      <Offcanvas.Body>
        <ListGroup variant="flush">
          {products.map((product, index) => {
            return (
              <ListGroup.Item variant="warning" id="card-item" key={index}>
                ...
                <div id="item-data">
                  ...
                  >
                    <img
                      ...
                      alt={`This is a ${product.title} image`}
                    />
